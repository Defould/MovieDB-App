...
//     try {
//       const response = await this.getData(`${this._apiBase}${this._guestUrl}?api_key=${this._apiKey}`);
...
//     const res = await this.getData(
//       `${this._apiBase}guest_session/${this._guestSessionId}/rated/movies?api_key=${this._apiKey}&language=en-US&page=${page}`,
//       this._options
//     );
//     if (res.results.length > 0) {
//       return [res.total_results, res.results.map(this._transformMovieData)];
...
//   async rateMovie(guestSessionId, id, rating) {
//     try {
//       let data = JSON.stringify({ value: rating });
